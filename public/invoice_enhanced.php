...
// Handle invoice header update
if ($id > 0 && $_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['update_invoice_header'])) {
    ...
    if (!$customer_name) {
        ...
        try {
            // Calculate commission amount based on percentage and current total
            $total_stmt = $pdo->prepare("SELECT final_total FROM invoices WHERE id = ?");
            $total_stmt->execute([$id]);
            $current_total = (float)$total_stmt->fetchColumn();
            
            if ($commission_percentage > 0) {
                $commission_amount = $current_total * ($commission_percentage / 100);
            }
            
            $stmt = $pdo->prepare("
                UPDATE invoices 
                SET invoice_dt = ?, customer_name = ?, firm_name = ?, phone = ?, customer_gst = ?, 
                    status = ?, salesperson_id = ?, commission_percentage = ?, commission_amount = ?, 
                    notes = ?, updated_at = datetime('now')
                WHERE id = ?
            ");
            
            if ($stmt->execute([$invoice_dt, $customer_name, $firm_name, $phone, $customer_gst, 
                               $status, $salesperson_id, $commission_percentage, $commission_amount, 
                               $notes, $id])) {
...
<?php else: ?>
    ...
    <!-- Invoice Header Edit Form -->
    <div class="card mb-4">
        <div class="card-header">
            <h6 class="mb-0"><i class="bi bi-pencil"></i> Edit Invoice Details</h6>
        </div>
        <div class="card-body">
            <form method="post" id="updateInvoiceForm">
                <input type="hidden" name="update_invoice_header" value="1">
                <div class="row g-3">
                    <div class="col-md-3">
                        <label class="form-label">Invoice Date</label>
                        <input type="date" class="form-control" name="invoice_dt" value="<?= h($invoice['invoice_dt']) ?>" required>
                    </div>
                    
                    <div class="col-md-3">
                        <label class="form-label">Customer Name</label>
                        <input type="text" class="form-control" name="customer_name" value="<?= h($invoice['customer_name']) ?>" required>
                    </div>
                    
                    <div class="col-md-3">
                        <label class="form-label">Firm Name</label>
                        <input type="text" class="form-control" name="firm_name" value="<?= h($invoice['firm_name'] ?? '') ?>">
                    </div>
                    
                    <div class="col-md-3">
                        <label class="form-label">Mobile Number</label>
                        <input type="tel" class="form-control" name="phone" value="<?= h($invoice['phone']) ?>" pattern="[0-9]{10}" required>
                    </div>
                    
                    <div class="col-md-3">
                        <label class="form-label">Customer GST</label>
                        <input type="text" class="form-control" name="customer_gst" value="<?= h($invoice['customer_gst'] ?? '') ?>">
                    </div>
                    ...
                    <div class="col-md-3">
                        ...
                        <select class="form-select" name="salesperson_id">
                            ...
                            <?php
                            // Get all active users for salesperson selection
                            $users_stmt = $pdo->query("SELECT id, username, role FROM users_simple WHERE active = 1 ORDER BY username");
                            $users = $users_stmt->fetchAll(PDO::FETCH_ASSOC);
                            foreach ($users as $user):
                            ?>
                                <option value="<?= $user['id'] ?>" <?= ($invoice['salesperson_id'] ?? $invoice['created_by']) == $user['id'] ? 'selected' : '' ?>>
                                    <?= h($user['username']) ?> (<?= h($user['role']) ?>)
                                </option>
                            <?php endforeach; ?>
                        </select>
                        <small class="text-muted">For commission calculation</small>
                    </div>
                    ...
                    <div class="col-md-3">
                        ...
                        <div class="input-group">
                            <input type="number" step="0.01" class="form-control" name="commission_percentage" 
                                   value="<?= h($invoice['commission_percentage'] ?? '0') ?>" min="0" max="100"
                    ...
                    <div class="col-md-3">
                        ...
                        <div class="input-group">
                            ...
                            <input type="number" step="0.01" class="form-control" name="commission_amount" 
                                   value="<?= h($invoice['commission_amount'] ?? '0') ?>" readonly
                                   id="commissionAmount">
                        </div>
                    </div>
