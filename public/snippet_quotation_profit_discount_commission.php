<?php
/* ---- DROP THIS BLOCK *AFTER* you compute $total_sell and $total_cost in quotation_profit.php ----
   Assumptions:
     - $total_sell  = grand total (₹) before any negotiation/discount
     - $total_cost  = total cost (₹) for the same items
     - $quote_id    = current quotation id (int)
     - $pdo         = Database::pdo()

   What this adds:
     - Discount: None / % / Amount
     - Commission: base (Profit or Sale), %, and "Commission To" person
     - Live preview of Net, Profit after discount, Commission amount, Profit after commission
     - Optional "Save & Store" persists to quotations table (requires migration 0006_*)
*/
if (!isset($quote_id)) {
