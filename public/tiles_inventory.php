...
if ($tile_id && $new_stock >= 0 && $adjustment_reason) {
    try {
        ...
        // Read current stock from summary
        $stCur = $pdo->prepare("SELECT COALESCE(current_stock, 0) FROM current_inventory_summary WHERE tile_id = ?");
        $stCur->execute([$tile_id]);
        $current_stock_val = (float)($stCur->fetchColumn() ?: 0);

        // Compute delta
        $delta = $new_stock - $current_stock_val;

        if (abs($delta) < 0.00001) {
            $message = "No change — current stock already at " . number_format($current_stock_val, 1);
        } else {
            ...
            $notes = "Reason: " . $adjustment_reason . ($adjustment_notes ? (" | " . $adjustment_notes) : "");

            $stAdj = $pdo->prepare("
                INSERT INTO inventory_transactions
                (tile_id, quantity_change, transaction_type, reference_id, transaction_date, notes, created_by)
                VALUES (?, ?, 'adjustment', NULL, datetime('now'), ?, ?)
            ");
            $stAdj->execute([$tile_id, $delta, $notes, $created_by]);

            // Refresh views/data
            InventoryUpdates::createInventoryViews($pdo);
            $message = "Adjusted Tile #$tile_id: " . number_format($current_stock_val,1) . " → " . number_format($new_stock,1) . " (Δ " . number_format($delta,1) . ")";
        }
    } catch (Exception $ex) {
        $error = "Stock adjustment failed: " . $ex->getMessage();
    }
} else {
...
// Initialize transport-inclusive inventory total for footer
$total_inventory_value_with_transport = 0;
...
<?php $total_inventory_value_with_transport += $available_value; ?>
                                    <?php endif; ?>
...
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        ...
        <small class="text-muted">
            Total Value: ₹<?= number_format($total_inventory_value_with_transport, 0) ?> | 
    ...
    <div class="table-responsive">
        <table class="table table-hover mb-0" id="tilesInventoryTable">
            <thead class="table-dark">
                <tr>
                    ...
                    <th style="width: 120px;">Available Boxes</th>
                                        <th style="width: 160px;">Cost per Box (incl. transport)</th>
            ...
            <tbody>
                ...
                <?php else: ?>
                    <?php $total_inventory_value_with_transport = 0; ?>
...
/* --- Added: compute avg cost per box INCLUDING transport (percentage or flat) --- */
$st_cost = $pdo->prepare("
    SELECT 
      CASE WHEN SUM(net_boxes) > 0 
           THEN SUM(net_boxes * eff_cost_per_box) / SUM(net_boxes)
           ELSE 0 END AS avg_cost_with_transport
    FROM (
      SELECT 
        (total_boxes * (100 - COALESCE(damage_percentage,0)) / 100.0) AS net_boxes,
        CASE 
          WHEN COALESCE(transport_percentage,0) > 0 
            THEN cost_per_box * (1 + transport_percentage/100.0)
          ELSE cost_per_box + COALESCE(transport_cost,0) / NULLIF(NULLIF(total_boxes,0),0)
        END AS eff_cost_per_box
      FROM purchase_entries_tiles
      WHERE tile_id = :tile_id
    ) x
");
$st_cost->execute([':tile_id' => (int)$item['tile_id']]);
$avg_cost_with_transport = (float)($st_cost->fetchColumn() ?: 0);

/* Cost per box to show on UI (transport-inclusive) */
$display_cost_per_box = $avg_cost_with_transport > 0 ? $avg_cost_with_transport : $avg_cost;

/* Available Boxes value = Available Boxes * Cost per Box (transport-inclusive) */
$available_value = $available_boxes * $display_cost_per_box;
...
<!-- Cost per Box -->
                            <td>
                                <div class="value-display">₹<?= number_format($display_cost_per_box, 2) ?></div>
                                <?php if ($item['sqft_per_box'] > 0): ?>
                            ...
                            <!-- Total Cost Value -->
                            <td>
                                <div class="value-display text-success">₹<?= number_format($available_value, 0) ?></div>
                                <?php if ($available_boxes > 0): ?>
                                    <small class="text-muted d-block">₹<?= number_format($total_cost_value / $available_boxes, 2) ?>/box incl. transport incl. transport avg</small>
                                <?php endif; ?>
                            </td>
            ...
            <tfoot class="table-light">
                <tr>
                    ...
                    <th class="text-primary fw-bold"><?= number_format($totals['total_boxes_available'], 1) ?></th>
                    <th>-</th>
                    <th class="text-success fw-bold">₹<?= number_format($total_inventory_value_with_transport, 0) ?></th>
                    <th>-</th>
                    <th>-</th>
                </tr>
            </tfoot>
